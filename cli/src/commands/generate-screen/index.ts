...
export const generateScreen = async (screenName: string, options: GenerateScreenOptions) => {
  logger.info('Nimbus TypeScript CLI: Generate Screen:')
  ...
  const configFileExists = await pathExists(`${cwd()}/nimbus-ts.json`)
  if (!configFileExists) {
    logger.error('This is not a Nimbus Backend TypeScript Project! (Configuration file "nimbus-ts.json" not found!)')
    exit(1)
  }
