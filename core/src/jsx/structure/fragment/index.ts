...
const createColumn = (children: any[]) => new Component({ namespace: 'layout', name: 'column', children })
...
export const nimbusFragmentFactory: FragmentFactory = (children: any[]) => {
  if (!children) return createColumn([])
  ...
  return childrenBuilt.length === 1 ? childrenBuilt[0] : createColumn(childrenBuilt)
}
